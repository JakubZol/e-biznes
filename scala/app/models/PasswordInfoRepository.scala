...
class PasswordInfoRepository @Inject() (userRepository: UserRepository)(implicit val classTag: ClassTag[PasswordInfo], ec: ExecutionContext)
  ...
  override def update(loginInfo: LoginInfo, passwordInfo: PasswordInfo): Future[PasswordInfo] = userRepository.find(loginInfo).flatMap {
    case Some(user) => userRepository.update(user.copy(password = Some(passwordInfo.password))).map(_.passwordInfo)
